...
import com.example.agenda.db.ConstantDb.CONTATOS_DB_NAME
import com.example.agenda.db.ConstantDb.CONTATOS_DB_TABLE
import org.jetbrains.anko.db.ManagedSQLiteOpenHelper
import org.jetbrains.anko.db.*

class BancoDadosHelper(context: Context) : ManagedSQLiteOpenHelper(ctx = context, name = CONTATOS_DB_NAME,  version = 1) {
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        db.dropTable("$CONTATOS_DB_TABLE", true)
        onCreate(db)
    }
}
